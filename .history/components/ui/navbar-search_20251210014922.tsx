...
export default function NavbarSearch() {
  ...
  const [results, setResults] = useState<{ subjects: any[]; topics: any[]; documents: any[] }>({
    subjects: [],
    topics: [],
    documents: [],
  });
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    const total = results.subjects.length + results.topics.length + results.documents.length;
    ...
    if (e.key === "ArrowDown") {
      ...
      const all = [...results.subjects, ...results.topics, ...results.documents];
      const item = all[highlight];
      let type: "subject" | "topic" | "document" = "subject";
      if (highlight < results.subjects.length) type = "subject";
      else if (highlight < results.subjects.length + results.topics.length) type = "topic";
      else type = "document";
          ...
          >
            ...
            {results.documents.map((doc, i) => (
              <button
                ...
                type="button"
                className={`flex w-full items-center gap-2 px-4 py-2 text-left text-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 focus:bg-blue-50 dark:focus:bg-blue-900/30 transition-colors ${
                  highlight === i + results.subjects.length + results.topics.length
                    ? "bg-blue-50 dark:bg-blue-900/30"
                    : ""
                }`}
                onClick={() => handleSelect(doc, "document")}
                onMouseEnter={() => setHighlight(i + results.subjects.length + results.topics.length)}
              >
