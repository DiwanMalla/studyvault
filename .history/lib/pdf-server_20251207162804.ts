...
export async function getSecurePdfPage(
  ...
  context.save();
  context.globalAlpha = 0.18; // More visible watermark

  // Diagonal Text
  context.translate(width / 2, height / 2);
  context.rotate(-Math.PI / 4);
  context.font = "bold 48px sans-serif";
  context.fillStyle = "rgba(200, 60, 60, 0.8)"; // Stronger red, more visible
  context.textAlign = "center";

  const timestamp = new Date().toISOString().split("T")[0];
  const watermarkText = `StudyVault — Do Not Share — ${timestamp} — MBBS`;

  // Repeat watermark more times for better coverage
  for (let i = -6; i <= 6; i++) {
